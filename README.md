...
* **Details:** A comprehensive system to Create, Read, Update, and Delete student records. Demonstrates encapsulation and robust `try-catch` exception handling for numerical inputs.

### 3️⃣ File-Based Library Management System
* **Focus:** File Handling (I/O), Persistent Storage, Hybrid Architecture.
* **Details:** An advanced library app that uses `BufferedReader` & `BufferedWriter` to save book records in a `.txt` file, ensuring no data is lost upon exit. Data is loaded into a `HashMap` at runtime for highly efficient O(1) processing speed.

### 4️⃣ Simple Quiz Application
* **Focus:** Collections Framework (ArrayList), Iteration, Dynamic Data.
